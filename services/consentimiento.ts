...
/**
 * Guardar consentimiento (formato idéntico al web - con firma digital)
 */
export async function guardarConsentimiento(
  ...
  ubicacion: UbicacionConsentimiento | null,
  ip: string | null,
  firmaDigital?: string | null  // Nuevo parámetro para firma
  ...
  try {
    ...
    // Datos del consentimiento (formato compatible con admin-consentimientos.js)
    const consentimientoData: any = {
      alumnoId: control,
      aceptado: true,
      ...
      ip: ip,
      userAgent: userAgent,
      version: '2.0'  // Actualizado a 2.0 con firma digital
    };
    
    // Agregar firma digital si existe
    if (firmaDigital) {
      consentimientoData.firmaDigital = firmaDigital;
      consentimientoData.tieneFirmaDigital = true;
      console.log('✅ Firma digital incluida');
    }
